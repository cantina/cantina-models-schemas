...
Schema.extend = function (/* target, source1, source2, etc. */) {
  ...
  return new Schema(extend.apply(null, [ true ].concat( [].slice.call(arguments).map(function (schema, idx) { return deinitialize(schema, idx > 0); }) )));
};
...
Schema.prototype.attach = function (collection, cb) {
  cb || (cb = function (err) { app.emit('error', err); });
  extend(collection, {
  ...
  if (indexes) {
    if (!indexers[collection.__type]) cb(new Error('Index type not supported'));
...
function deinitialize (schema, clone) {
  if (!clone) {
    ...
    // shallow copy excluding uninheritableProps
    return Object.keys(schema).reduce(function (copy, prop) {
      if (uninheritableProps.indexOf(prop) === -1) {
