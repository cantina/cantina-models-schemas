...
var Schema = module.exports = function (schema) {
  ...
  this.strict = typeof schema.strict === 'undefined' ? true : schema.strict;
};
  ...
  if (this.properties) {
    ...
    if (self.strict) {
      defs.save.push(function (model) {
        function filterProperties (attrs, schemaProperties) {
          Object.keys(attrs).forEach(function (prop) {
            if (!schemaProperties[prop]) {
              delete attrs[prop];
            }
            else if (!schemaProperties[prop].type) {
              filterProperties(attrs[prop], schemaProperties[prop]);
            }
          });
        }
        filterProperties(model, self.properties);
      });
    }
  }
